...
helm list --all-namespaces | grep -q 'prometheus-adapter'

echo "[debug] setup prometheus-community/prometheus-adapter"
helm upgrade \
...
helm list --all-namespaces --filter prometheus-adapter

...
helm list --all-namespaces | grep -q 'prometheus-node-exporter'

echo "[debug] setup prometheus-community/prometheus-node-exporter"
helm upgrade \
...
helm list --all-namespaces --filter prometheus-node-exporter

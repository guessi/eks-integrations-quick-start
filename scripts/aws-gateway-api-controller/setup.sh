...
# ref: https://github.com/aws/aws-application-networking-k8s/blob/v2.0.1/docs/guides/deploy.md
...
if [ $? -ne 0 ]; then
  kubectl apply -f https://raw.githubusercontent.com/aws/aws-application-networking-k8s/v${APP_VERSION}/files/controller-installation/deploy-namesystem.yaml
else
...
if [ $? -ne 0 ]; then
  ...
  curl -fsSL https://raw.githubusercontent.com/aws/aws-application-networking-k8s/v${APP_VERSION}/files/controller-installation/recommended-inline-policy.json -O
...
if ! kubectl get GatewayClass | grep -q 'amazon-vpc-lattice'; then
  ...
  kubectl apply -f https://raw.githubusercontent.com/aws/aws-application-networking-k8s/v${APP_VERSION}/files/controller-installation/gatewayclass.yaml
